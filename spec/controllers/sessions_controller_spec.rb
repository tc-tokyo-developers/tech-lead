  ...
  describe '#callback' do
    ...
    context 'when session[:request_type] is student' do
      ...
      context 'when requested user has not registered' do
        ...
        it 'assigns the requested user id to cookies.signed(:student_id)' do
          subject
          new_student = Student.find_by(uid: uid)
          expect(cookies.signed[:student_id]).to eq new_student.id
        end
      ...
      context 'when requested student has already registered' do
        ...
        it 'assigns the requested student id to cookies.signed(:student_id)' do
          expect(cookies.signed[:student_id]).to eq student.id
        end
    ...
    context 'when session[:request_type] is mentor' do
      ...
      context 'when requested mentor has not registered' do
        ...
        it 'assigns the requested mentor id to cookies.signed(:mentor_id)' do
          subject
          new_mentor = Mentor.find_by(uid: uid)
          expect(cookies.signed[:mentor_id]).to eq new_mentor.id
        end
      ...
      context 'when requested mentor has already registered' do
        ...
        it 'assigns the requested mentor id to cookies.signed(:mentor_id)' do
          expect(cookies.signed[:mentor_id]).to eq mentor.id
        end
